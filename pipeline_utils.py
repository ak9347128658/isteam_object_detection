...
class Embedder:
    ...
    def embed_bgr(self, bgr) -> "Any":
        """Embed a single OpenCV BGR image (numpy) without a disk round-trip."""
        import numpy as np
        from PIL import Image
...
def process_frames_sequential(frames, detector, embedder, cfg,
                              crops_dir: str | Path) -> list["Detection"]:
    """
    Walk frames IN ORDER and, for each detection:
      1. Drop it if the crop is too blurry (blur = low Laplacian variance).
      2. Skip it if it is essentially the SAME image as the last kept crop of
         the same label in this consecutive run (CLIP cosine >= threshold).
         The user's rule: if a sequential same-image match is >= 0.60, do not
         add it again.
    Only kept detections get a crop saved to disk. Returns the kept detections.

    Config:
      dedup.sequential_skip_similarity   (default 0.60)  -> the 60% rule
      dedup.blur_min_sharpness           (default 100.0) -> remove-if-blurrier
    """
    import cv2
    import numpy as np
    ...
    for fr in frames:
        for det in detector.detect_frame(fr):
            ...
            # (1) blur removal: skip crops that are too blurry.
            if blur_min > 0:
            ...
            vec = embedder.embed_bgr(region)
            prev = last_vec.get(det.label)
            if prev is not None:
                sim = float(np.dot(vec, prev))
                if sim >= seq_thresh:
            ...
            detector.save_crop(fr, det, crops_dir)
            det.embedding_id = len(kept)
            kept.append(det)
            last_vec[det.label] = vec
